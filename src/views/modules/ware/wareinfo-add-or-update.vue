...
<script>
import { reqWareById } from '@/api/modules/ware'
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          // 修改，回显商品信息到表单
          this.$API.ware.reqWareById(this.dataForm.id).then(
            Response => {
              this.dataForm.name = Response.data.name
              this.dataForm.address = Response.data.address
              this.dataForm.areacode = Response.data.areacode
              this.$message.success('查询单个仓库成功～')
            }
          )
          /* this.$http({
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$API.ware.reqAddOrEditWare(this.dataForm).then(
            Response => {
              this.$message({
                message: '操作成功',
                type: 'success',
