...
export default {
  ...
  methods: {
    ...
    getMemberLevels() {
      this.$API.level.reqGetMemberLevelList(1, 500, null).then(
        Response => {
          this.dataResp.memberLevels = Response.data.records
        }
      )
      /* this.$http({
    ...
    getShowSaleAttr() {
      if (!this.dataResp.steped[1]) {
        this.$API.attr.reqGetAttrList(this.spu.catalogId,1,500,null,'sale').then(
          Response=>{
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$API.attrGroup.reqGetAttrGroupWithAttr(this.spu.catalogId).then(
          Response => {
            //先对表单的baseAttrs进行初始化
    ...
    submitSkus() {
      ...
      this.$confirm('将要提交商品数据，需要一小段时间，是否继续?', '提示', {
        ...
        .then(() => {
          this.$API.spuInfo.reqSaveSpuInfo(this.spu).then(
            Response=>{
              this.$message({
                type: 'success',
